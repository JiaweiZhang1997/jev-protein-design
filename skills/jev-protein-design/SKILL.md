...
7. If the user asks for structures, show the GFP and RuBisCO worked comparisons in the README. Compare a Jev candidate's Boltz prediction with a real protein's experimental PDB chain, show both as separate cartoon structures, and report sequence alignment, TM-score, confidence, and limitations. For new Boltz predictions, use a key from the user's ignored local environment and request the official cost estimate first. A structure prediction or similarity score does not establish the requested biological function.
